...
async function connectChannel(channelName) {
  ...
  _channel.on("broadcast", { event: "dispatch.request" }, ({ payload }) => {
    // Hand off to the dispatch handler in server.js. The handler MUST
    // call sendOutbound(name, payload) with the same channel name and
    // matching request_id to stream the response back.
    try {
