...
// File writer. Same sandbox rules as the reader, plus a size cap so a
// runaway client can't fill the disk. Creates parent dirs if missing
// (file editor will only touch existing files, but the safety check is
// cheap). Returns { bytesWritten } on success.
async function writeFileText(cwd, rel, contents) {
  if (!rel) {
    const err = new Error("rel required");
    err.code = "EINVAL";
    throw err;
  }
  if (typeof contents !== "string") {
    const err = new Error("contents must be a string");
    err.code = "EINVAL";
    throw err;
  }
  const MAX_BYTES = 4 * 1024 * 1024; // 2x the read cap — saves can grow a bit
  const bytes = Buffer.byteLength(contents, "utf8");
  if (bytes > MAX_BYTES) {
    const err = new Error(`file too large (${bytes} bytes, max ${MAX_BYTES})`);
    err.code = "E2BIG";
    throw err;
  }
  const target = await safeResolve(cwd, rel);
  // Reject binary or unrecognized extensions — same policy as the reader.
  // Stops the editor from accidentally clobbering a .png with text data.
  const filename = path.basename(target);
  if (!isTextExt(filename)) {
    const err = new Error("refusing to write a binary/unrecognized file type");
    err.code = "EBINARY";
    throw err;
  }
...
function mountProjectFiles(app) {
  ...
  // Save back. Body: { cwd, rel, contents }. Same sandbox rules as
  // /api/files/read — and the same text-only allowlist (no clobbering
  // binaries). Returns the post-write size.
  app.post("/api/files/write", async (req, res) => {
    const { cwd, rel, contents } = req.body || {};
    if (!cwd || !rel) return res.status(400).json({ ok: false, error: "cwd + rel required" });
    if (typeof contents !== "string") return res.status(400).json({ ok: false, error: "contents required" });
    if (!fs.existsSync(cwd)) return res.status(404).json({ ok: false, error: "cwd not found" });
    try {
      // Capture the BEFORE so the activity feed can render a diff. If
      // the file didn't exist yet (new file via editor), before="".
      let before = "";
      try { before = await readFileText(cwd, rel).then((d) => d.contents); }
      catch { /* new file or unreadable — before stays empty */ }
      const result = await writeFileText(cwd, rel, contents);
      // Fire-and-forget — never let activity logging block the save
      // response or surface as a save failure.
      try {
        const { recordUserEdit } = require("./activity");
        recordUserEdit({ cwd, relPath: rel, before, after: contents }).catch(() => {});
      } catch { /* activity module optional */ }
      try {
        require("./analytics").emit("dispatch_sent", {
          mode: "editor",
          metadata: { rel, bytes: result?.bytesWritten || 0 },
        });
      } catch { /* ignore */ }
...
module.exports = { mountProjectFiles, listDir, readFileText, writeFileText };
